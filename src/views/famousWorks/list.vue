...
<script>
import famousWorksApi from "@/api/famousWorks";
...
export default {
  ...
  methods: {
    ...
    async deleteNovel(id) {
      ...
      const res = await famousWorksApi.delById(id);
    ...
    async getArticlInfo(id) {
      ...
      const { data: res } = await famousWorksApi.getArticle(id);
      this.articleForm = res.article;
      console.log(res);
    },
    async getAll() {
      const { data: res } = await famousWorksApi.getAll();
      console.log(res.famousWorks);
      this.novelList = res.famousWorks;
    ...
    async articleSave() {
      if (this.form.id) {
        await famousWorksApi.updateArticle(this.articleForm);
        this.$message.success("編輯成功");
      }else{
        await famousWorksApi.addArticle(this.articleForm);
        this.$message.success("添加成功");
      }
    },
    async deleteAritcle(id){
      await famousWorksApi.delArticleById(id);
    },
    async save() {
      if (this.form.id) {
        console.log(this.form);
        const res = await famousWorksApi.update(this.form);
        this.$message.success("編輯成功");
        this.dialogVisible = false;
        this.clearData();
        this.getList();
      } else {
        const res = await famousWorksApi.add(this.form);
        this.$message.success("添加成功");
        this.clearData();
      }
    },
    async getEditInfo(id) {
      ...
      const { data: res } = await famousWorksApi.getById(id);
      console.log("cdd" + res);
      this.novelDefaultFile.push({
        url: "http://localhost:53021/web" + res.famousWorks.imgLink,
      });
      this.form = res.famousWorks;
      this.dialogImageUrl = res.famousWorks.imgLink;
